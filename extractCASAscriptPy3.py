...
'''
#HS_Nov2019 Updated code to work with Python3.
# Updated Print function to use Print(). Updated urllib and urllib2 to use urllib.request. Updated string.expandtabs(line) to str.expandtabs(line)
# Updated print >>f,line to print(line,file=f). I am not sure line 655-656,666,675 is correct and did not test this path.
...
def main( URL, options ):
    ...
    * options = options object created by optparse.  If options.benchmark
    ...
    if ( URL[:4].upper() == 'HTTP' ):
        ...
        responseLines = localFile.read().decode("utf-8").split("\n")
